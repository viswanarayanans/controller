...
with open('odom_smc_pipe.txt') as file:
	for i in file.readlines():
		k = i.split(':')
...
with open('odom_rsb_pipe.txt') as file:
	for i in file.readlines():
		k = i.split(':')
...
with open('traj_rsb_pipe.txt') as file:
	for i in file.readlines():
		k = i.split(':')
...
with open('error_rsb_pipe.txt') as file:
	for i in file.readlines():
		k = i.split(':')
...
with open('error_smc_pipe.txt') as file:
	for i in file.readlines():
		k = i.split(':')
...
plt.axis([0, odom_smc_time[-1], -1, 13])
...
# plt.ylabel('z')
plt.axis([0, odom_smc_time[-1], -0.1, 2])
plt.legend(loc=2, prop={'size': 8}, ncol=4)
# plt.xlabel('time')
...
fig.tight_layout()
plt.show()
fig.savefig('pipe_traj_exp1.png')
...
plt.axis([0, err_smc_time[-1], -0.4, 0.6])
...
plt.xlabel('time (s)')
plt.axis([0, err_smc_time[-1], -0.6, 0.2])
plt.legend(loc=3, prop={'size': 10}, ncol=4)

fig.tight_layout(h_pad=0.4)
plt.show()
fig.savefig('pipe_pos_error_exp1.png')
...
plt.title('Moment: z')
plt.xlabel('time (s)')
plt.axis([0, err_smc_time[-1], -0.03, 0.03])
plt.legend(loc=3, prop={'size': 10}, ncol=4)

fig.tight_layout(h_pad=0.4)
plt.show()
fig.savefig('pipe_control_op_exp1.png')
...
fig.tight_layout(h_pad=0.4)
plt.show()
fig.savefig('pipe_error_att_exp1.png')
...
# index = np.where((np.array(err_smc_time)>20) & (np.array(err_smc_time)<50))[0]
# print(index.astype(int))

# fig = plt.figure()
# plt.plot(err_smc_time[index[0]:index[-1]], err_smc_x[index[0]:index[-1]], '--', c='orange', label="SMC", lw=2)
# plt.plot(err_rsb_time[index[0]:index[-1]], err_rsb_x[index[0]:index[-1]], label="RSB(proposed)", lw=2)
# plt.plot(np.array([20, 50]), np.array([0.3,0.3]), 'r-' , label='bounds', lw=0.5)
# plt.plot(np.array([20, 50]), np.array([-0.3,-0.3]), 'r-' , lw=0.5)
# # plt.plot(err_sb_time[:len(err_sb_time)], err_sb_y[:len(err_sb_time)], label="SB", lw=2)
# # plt.plot([0,err_pd_time[-1]], [0,0])
# # plt.ylabel('y_err')
# plt.title('Position error: y (m)')
# plt.axis([20, 50, -0.5, 0.5])
# plt.show()

#include "rrc_control/asmc_mavros_controller.h"
...
using namespace std;
using namespace rrc_control;
// Constructor
ASmcMavrosController::ASmcMavrosController(const ros::NodeHandle &nh, const ros::NodeHandle &nh_private)
    : nh_(nh),
  ...
  mavstateSub_ =
      nh_.subscribe("mavros/state", 1, &ASmcMavrosController::mavstateCallback, this, ros::TransportHints().tcpNoDelay());
  mavposeSub_ = nh_.subscribe("mavros/local_position/pose", 1, &ASmcMavrosController::mavposeCallback, this,
                              ros::TransportHints().tcpNoDelay());
  mavtwistSub_ = nh_.subscribe("mavros/local_position/velocity_local", 1, &ASmcMavrosController::mavtwistCallback, this,
                               ros::TransportHints().tcpNoDelay());
  ctrltriggerServ_ = nh_.advertiseService("tigger_rlcontroller", &ASmcMavrosController::ctrltriggerCallback, this);
  cmdloop_timer_ = nh_.createTimer(ros::Duration(0.01), &ASmcMavrosController::cmdloopCallback,
                                   this);  // Define timer for constant loop rate
  statusloop_timer_ = nh_.createTimer(ros::Duration(1), &ASmcMavrosController::statusloopCallback,
                                      this);  // Define timer for constant loop rate
  ...
  land_service_ = nh_.advertiseService("land", &ASmcMavrosController::landCallback, this);
  ...
  nh_private_.param<double>("max_thrust_", max_thrust_, 30.0);
  ...
  GetRosParameter(nh_private_, "lam_p/x", 1.5, &lam_p.x());
  GetRosParameter(nh_private_, "lam_p/y", 1.5, &lam_p.y());
  GetRosParameter(nh_private_, "lam_p/z", 1.5, &lam_p.z());

  GetRosParameter(nh_private_, "theta_p/x", 1.0, &theta_p.x());
  GetRosParameter(nh_private_, "theta_p/y", 1.0, &theta_p.y());
  GetRosParameter(nh_private_, "theta_p/z", 1.0, &theta_p.z());

  GetRosParameter(nh_private_, "alpha_p/x", 10.0, &alpha_p.x());
  GetRosParameter(nh_private_, "alpha_p/y", 10.0, &alpha_p.y());
  GetRosParameter(nh_private_, "alpha_p/z", 10.0, &alpha_p.z());

  GetRosParameter(nh_private_, "alphaM", 10.0, &alphaM);
  
  GetRosParameter(nh_private_, "hatKp/x", 0.1, &hatKp.x());
  GetRosParameter(nh_private_, "hatKp/y", 0.1, &hatKp.y());
  GetRosParameter(nh_private_, "hatKp/z", 0.1, &hatKp.z());
  
  GetRosParameter(nh_private_, "hatM", 0.1, &hatM);
...
ASmcMavrosController::~ASmcMavrosController() {
  // Destructor
}

void ASmcMavrosController::targetCallback(const geometry_msgs::TwistStamped &msg) {
...
void ASmcMavrosController::multiDOFJointCallback(const trajectory_msgs::MultiDOFJointTrajectory &msg) {
...
void ASmcMavrosController::mavposeCallback(const geometry_msgs::PoseStamped &msg) {
...
void ASmcMavrosController::mavtwistCallback(const geometry_msgs::TwistStamped &msg) {
  mavVel_ = toEigen(msg.twist.linear);
  mavRate_ = toEigen(msg.twist.angular);
}

bool ASmcMavrosController::landCallback(std_srvs::SetBool::Request &request, std_srvs::SetBool::Response &response) {
  node_state = LANDING;
  return true;
}

void ASmcMavrosController::cmdloopCallback(const ros::TimerEvent &event) {
  switch (node_state) {
...
void ASmcMavrosController::mavstateCallback(const mavros_msgs::State::ConstPtr &msg) { current_state_ = *msg; }

void ASmcMavrosController::statusloopCallback(const ros::TimerEvent &event) {
  if (sim_enable_) {
...
void ASmcMavrosController::pubReferencePose(const Eigen::Vector3d &target_position, const Eigen::Vector4d &target_attitude) {
...
void ASmcMavrosController::pubRateCommands(const Eigen::Vector4d &cmd) {
...
void ASmcMavrosController::pubPoseHistory() {
  nav_msgs::Path msg;
...
void ASmcMavrosController::pubSystemStatus() {
...
void ASmcMavrosController::appendPoseHistory() {
...
geometry_msgs::PoseStamped ASmcMavrosController::vector3d2PoseStampedMsg(Eigen::Vector3d &position,
...
void ASmcMavrosController::computeBodyRateCmd(Eigen::Vector4d &bodyrate_cmd, const Eigen::Vector3d &target_pos,
  ...
  const Eigen::Vector3d sp = vel_error + theta_p.cwiseProduct(pos_error);
  ...
  if(armed){
	  hatKp += (sp - alpha_p.cwiseProduct(hatKp))*dt;
	  hatM += (-sp[2] - alphaM*hatM)*dt;
  }
  // ROS_INFO("%f",hatM);

  Eigen::Vector3d del_tau_p, zeta_p;
  zeta_p = hatKp;
  del_tau_p<<zeta_p[1]*Sigmoid(sp[0],1), zeta_p[1]*Sigmoid(sp[1],1), zeta_p[2]*Sigmoid(sp[2],1);


  tau_p = -lam_p.cwiseProduct(sp) - del_tau_p - hatM*g_;

  // Eigen::Vector3d a_fb =
  //     Kpos_.asDiagonal() * pos_error + Kvel_.asDiagonal() * vel_error;  // feedforward term for trajectory error
  if (tau_p.norm() > max_thrust_)
    tau_p = (max_thrust_ / tau_p.norm()) * tau_p;  // Clip acceleration if reference is too large
  ...
  q_des = acc2quaternion(tau_p, mavYaw_);

  if (ctrl_mode_ == ERROR_GEOMETRIC) {
    bodyrate_cmd = geometric_attcontroller(q_des, tau_p, mavAtt_);  // Calculate BodyRate

  } else {
    bodyrate_cmd = attcontroller(q_des, tau_p, mavAtt_);  // Calculate BodyRate
  }
}

Eigen::Vector4d ASmcMavrosController::acc2quaternion(const Eigen::Vector3d &vector_acc, const double &yaw) {
  Eigen::Vector4d quat;
...
Eigen::Vector4d ASmcMavrosController::attcontroller(const Eigen::Vector4d &ref_att, const Eigen::Vector3d &ref_acc,
  ...
  zb = rotmat.col(2);
  ratecmd(3) = tau_p.dot(zb);
      // std::max(0.0, std::min(1.0, tau_p.dot(zb)));  // Calculate thrust

  return ratecmd;
}

Eigen::Vector4d ASmcMavrosController::geometric_attcontroller(const Eigen::Vector4d &ref_att, const Eigen::Vector3d &ref_acc,
                                                       Eigen::Vector4d &curr_att) {
  // Geometric attitude controller
  ...
  zb = rotmat.col(2);
  ratecmd(3) =
      std::max(0.0, std::min(1.0, 0.05*tau_p.dot(zb)));  // Calculate thrust

  return ratecmd;
}

bool ASmcMavrosController::ctrltriggerCallback(std_srvs::SetBool::Request &req, std_srvs::SetBool::Response &res) {
...
// void ASmcMavrosController::dynamicReconfigureCallback(geometric_controller::GeometricControllerConfig &config,
//                                                uint32_t level) {
